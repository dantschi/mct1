...
## Input: Schalter und Taster

- Beispiel: D-Pad an `0xF0000000`
- `lw` liefert 32-Bit-Wort als Bit-Map
- Jedes Bit = ein physikalischer Schalter (1 = gedrückt)

| Bits 31:4 | Bit 3 | Bit 2 | Bit 1 | Bit 0 |
|-----------|-------|-------|-------|-------|
| 0… | Unten | Oben | Rechts | Links |

::: notes
Nicht als Dezimalzahl lesen: Bit 0 = Links, Bit 2 = Oben, …
:::

## Taster auslesen und maskieren

- Problem: mehrere Tasten gleichzeitig (z. B. `0b0101`)
...
# Bit 2 (Oben): Maske 4 = 0b0100
andi t1, t0, 4
bne t1, zero, Jump
```

::: notes
Diagonal-Druck ergibt Wert 5, nicht 4. AND filtert auf eine saubere 4 oder 0.
:::
...
Service 5 ist ggf. atoi (String	o Int), kein Tastatur-Input.
```
...
- Kernel darf Caller-Saved (`t0`–`t6`, `a0`–`a7`) zerstören
- Schleifenzähler in `t0` + Print in der Schleife = Zähler weg
- Lösung: vor `ecall` auf Stack sichern oder `s0`–`s11` nutzen

::: notes
Klassische Studenten-Falle: nach Syscall `t`-Register nie wieder vertrauenswürdig; Zähler in `s`-Registern halten.
:::
